        ...
        try {
            ...
            loadScript(this, SOCKETIO).then(() => { 
                ...
                websocket.on("connect", (data) => {
                    ...
                    emitEvent(websocket, AgentAssistLabels.SET_INTERACTION_CONTEXT, AgentAssistEvents.set_interaction_context("986753462", "123456543", "member_123", "test@test.com", "member_123"));
                ...
                websocket.on(AgentAssistLabels.HISTORICAL_INTERACTION_SUMMARY, (data) => {
                    ...
                    console.log('agentAssistUtils | createWebSocketIoClient | historical_interaction_summary: ' + JSON.stringify(data));
                ...
                websocket.on(AgentAssistLabels.KNOWLEDGE_CARD, (data) => {
                    ...
                    data = JSON.parse(data);
                    console.log('agentAssistUtils | createWebSocketIoClient | knowledge_card data', JSON.stringify(data));
                ...
                websocket.on(AgentAssistLabels.ASK_ME_ANYTHING_RESPONSE, (data) => {
                    ...
                    data = JSON.parse(data);
                    console.log('agentAssistUtils | createWebSocketIoClient | ask_me_anything_response: ' + JSON.stringify(data));
                ...
                websocket.on(AgentAssistLabels.SET_CUSTOMER_CONTEXT, (data) => {
                    ...
                    data = JSON.parse(data);
                    console.log('agentAssistUtils | createWebSocketIoClient | set_customer_context: ' + JSON.stringify(data));
                });
...
export async function emitEventWithoutACK(websocket, eventType) {
    ...
    if(websocket != null && websocket.connected){
        while (attempts <= maxAttempts && websocket.connected) {
            try {
                console.log("agentAssistUtils | emitEventWithoutACK | emitting ", eventType + " @ ", new Date().toISOString());
                const result = await new Promise((resolve, reject) => {
                    websocket.timeout(10000).emit(eventType, (error, result, data) => {
                        if(error) {
                            console.log('agentAssistUtils | emitEventWithoutACK | eventType: ' + eventType + ', Received error or timeout @', new Date().toISOString());
                            //console.log(JSON.stringify(error));
                            reject(error);
                        }
                        else {
                            console.log('agentAssistUtils | emitEventWithoutACK | eventType: ' + eventType + ', Received ack: ' + result + '@ ', new Date().toISOString());
                ...
                console.log('agentAssistUtils | emitEventWithoutACK | ' + eventType + ' | Attempt #' + attempts + ' | Error occurred attempting to emit eventType event: ' + error);
                attempts++;
            }
        }
        if(attempts > maxAttempts) {
            console.log('agentAssistUtils Panel | emitEventWithoutACK | Failed to send ' + eventType + ' event after ' + maxAttempts + ' attempts.');
            //TODO Log Errors here
            if(eventType == AgentAssistLabels.AGENT_FEEDBACK) {
                const messageContext = createMessageContext()
                publish(messageContext, AGENTASSISTLMS, AgentAssistEvents.aa_lms_event(AgentAssistLabels.AGENT_FEEDBACK_ERROR, {foo:"bar"} )); 
            }
        }
    }
    else {
            ...
            try {
                ...
                const result = await new Promise((resolve, reject) => {
                    websocket.timeout(10000).emit(eventType, eventData, (error, result, data) => {
                        if(error) {
                            console.log('agentAssistUtils | emitEvent | eventType: ' + eventType + ', Received error or timeout @', new Date().toISOString());
                            //console.log(JSON.stringify(error));
                            reject(error);
                        }
                        else {
                            console.log('agentAssistUtils | emitEvent | eventType: ' + eventType + ', Received ack: ' + result + '@ ', new Date().toISOString());
                            console.log('agentAssistUtils | emitEvent | eventType: ' + eventType + ', data: ' + JSON.stringify(data));
                ...
                console.log('agentAssistUtils | emitEvent | ' + eventType + ' | Attempt #' + attempts + ' | Error occurred attempting to emit eventType event: ' + error);
                attempts++;
            }
        }
        if(attempts > maxAttempts) {
            console.log('agentAssistUtils Panel | emitEvent | Failed to send ' + eventType + ' event after ' + maxAttempts + ' attempts.');
            //TODO Log Errors here
            if(eventType == AgentAssistLabels.AGENT_FEEDBACK) {
                publish(messageContext, AGENTASSISTLMS, AgentAssistEvents.aa_lms_event(AgentAssistLabels.AGENT_FEEDBACK_ERROR, eventData)); 
            }
        }
    }
    else {
...
export const AgentAssistEvents = {
            ...
            feedback: {
                ...
                properties: {
                    ...
                    rating: {
                        ...
                        type: "boolean",
                        description: "Rating given by the agent, e.g., true for positive feedback and false for negative feedback"
                    }
                }
            }
        }
            ...
            card_metadata: {
                ...
                properties: {
                    ...
                    member_id: {
                        value: "string",
                        type: "string",
                        description: "The ID on an insurers card - there can be multiple members on a single card, so this is not unique to each person"
                    },
                    customer_id: {
                        value: "string",
                        type: "string",
            ...
            card_metadata: {
                ...
                properties: {
                    interaction_id: {
                        ...
                        interaction_id_type: "voice",
                        type: "string",
                        description: "Interaction ID of the interaction, if voice this is genesys_interaction_id, if chat tbd"
                    }
                },
    ...
    set_customer_context: (customer_type, enterprise_person_id, customer_id, get_historical_interactions_flag, interaction_id) => ({
        version: "0.1",
