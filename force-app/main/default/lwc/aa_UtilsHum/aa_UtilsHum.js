    ...
    async emitEvent(eventType, eventData) {
        ...
        if(this.websocket != null && this.websocket?.connected){
                ...
                try {
                    ...
                    result = await new Promise((resolve, reject) => {
                        this.websocket.timeout(timeout).emit(eventType, eventData, (error, result, data) => {
                            if(error) {
                                console.log('aa_UtilsHum | emitEvent | eventType:' + eventType + '. Received error or timeout @', new Date().toISOString());
                                reject(error);
                            }
                            else {
                    ...
                    console.log('aa_UtilsHum | emitEvent | ' + eventType + ' | Attempt #' + attempts + ' | Error occurred attempting to emit eventType event: ' + error);
            ...
            if (eventType == AgentAssistLabels.SET_INTERACTION_CONTEXT) {
                ...
                const index = this.eventQueue.findIndex(item => item.eventType == AgentAssistLabels.SET_INTERACTION_CONTEXT);
                if (index > -1) {
...
export const AgentAssistSplunkLoggingUtils = {
    splunk_logging_context: (event_type, triggering_component, subcomponent_name, event_title, correlation_id, requested_data, salesforce_user_id) => ({
